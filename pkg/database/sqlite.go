...
func (s *SQLiteBackend) CreateDatabase(db *sql.DB) error {
	sqlStmt := `
...
func (s *SQLiteBackend) UpdateDatabase(db *sql.DB, nodeMetrics *v1beta1.NodeMetricsList, podMetrics *v1beta1.PodMetricsList) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
...
func (s *SQLiteBackend) CullDatabase(db *sql.DB, window *time.Duration) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
